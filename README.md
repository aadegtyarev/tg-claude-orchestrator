...
⏳ Ждёт фоновую задачу (⏭ — разблокировать ввод)
[⏹ Стоп-отчёт] [⏭ Разблокировать] [⛔ Прервать ход]
```
...
- Три кнопки управления ходом (одна семантика во всех интерфейсах):
  - **⏹ Стоп-отчёт** — мягко: push «сверни работу и отчитайся», модель
    прочитает, когда доберётся.
