...
| `MODULES` | `wallet` при `bwrap` | Модули: `wallet` (кошелёк). Не задано → включён при `SANDBOX=bwrap`; `MODULES=` — отключить. Кошелёк **требует `SANDBOX=bwrap`** и вне её не включается даже явно (см. ниже) |
...
> включается даже при явном `MODULES=wallet`** (иначе был бы «включён и молча
