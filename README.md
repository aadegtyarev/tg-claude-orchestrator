...
- секреты и policy — `WALLET_SECRETS_FILE` (TOML, 0600, вне allowlist
  ...
  логин/пароль; хранится вне чата/репо, выдаётся `wallet get`/`env` или
  авто-инъектом в env сессии);
- в сессии доступен CLI: `wallet ls`, `wallet run <имя> -- gh pr list` —
  демон ядра исполняет команду **на хосте** с секретом в env ребёнка;
- токен туда, где env не читается: плейсхолдеры `{{secret}}` (в аргумент, напр.
  `curl -H "Authorization: Bearer {{secret}}"`) и `{{secret_file}}` (во временный
  файл, напр. `ssh -i {{secret_file}}`) — подставляет демон, значение к модели не
  попадает;
...
хостового терминала), `/skills`, `/chat_id`, `/help`.
