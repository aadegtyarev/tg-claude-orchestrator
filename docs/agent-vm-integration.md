...
**3. ✅ Кошелёк в VM — путь A ПОДТВЕРЖДЁН ЖИВЬЁМ, кода не требует.** У
`agent-vm claude` есть флаг `--no-git` («don't inject host gh/git credentials
into the guest»), т.е. инъекция кредов включена ПО УМОЛЧАНИЮ, а мы `--no-git` не
передаём. Замер в госте на этом репозитории:
...
Это ровно гарантия кошелька (работа с git/gh «как на хосте» без выдачи значения
токена модели), но её обеспечивает прокси agent-vm, а не наш демон. Allow-list
репозиториев берётся из `git remote -v` в cwd — для сессии с линком на проект
это то, что нужно; расширять — `--repo OWNER/REPO`. Наш модуль `wallet` под
agent-vm корректно НЕ включается (`Config._default_modules`: без bwrap кошелёк
не страхует). **Путь B (проброс демона в гостя) закрыт как ненужный** — остаётся
только если понадобятся inject/shared-секреты произвольных env, не покрытые
HTTP-прокси; его блокер (пустой env гостя) в силе.

**Остаётся:** транскрипты/stats внутри VM (проблема #3).
