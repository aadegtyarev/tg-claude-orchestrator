...
**Путь B (проброс демона в гостя) остаётся нужным**, и его блокер подтверждён
замером (2026-07-21): у `agent-vm` НЕТ флага проброса env (`-e`/`--env` нет ни
в одной подкоманде), хостовый env в гостя не течёт (`WALLET_MARKER`/`FOO` →
`unset`), runtime-хука тоже нет. Без механизма передать `$NAME`-маркеры и
`~/.wallet.json` в гостя путь B не собирается. Кандидаты (не проверены): свой
wrapper-скрипт как agent-cmd, кастомный образ, либо `--mount` только
`.wallet.json`+`.wallet-bin` с адресом демона `host.microsandbox.internal`
(сеть для B, напомним, уже решена — демон может остаться на 127.0.0.1).

**Остаётся:** путь B кошелька (выше); транскрипты/stats внутри VM (проблема #3).
...
**Рекомендация:** A (прокси agent-vm) как дефолт для git/gh — он это уже
закрывает (подтверждено живьём, см. Шаг 2). Но A ≠ кошелёк: он не покрывает
ssh/scp, произвольные токены для curl/CLI, inject-секреты, вымарывание значений
в выводе и policy per-session/per-command. Поэтому **B нужен для всего
остального**, и до него VM-режим по секретам беднее bwrap — таблицу покрытия и
подтверждённый блокер B (нет проброса env в гостя) см. в разделе Шага 2.
