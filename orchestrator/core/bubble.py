...
работает над предыдущим), старый бабл ЗАМОРАЖИВАЕТСЯ на месте (кнопка действия
«📋 Отчёт» снимается, дальше не редактируется), а новый открывается и копит
...
TEXT_LIMIT = 3800
EDIT_INTERVAL = 1.5
# Сколько flush'ей подряд без единой доставки терпим, прежде чем перестать
# само-перепланировать _flush. Без этого потолка устойчиво-недоставляемый бабл
# (Telegram навсегда отвергает правку: 400 «can't parse entities», сообщение
# стало нередактируемым, длительный 5xx) крутил бы _flush каждые EDIT_INTERVAL
# до закрытия бабла — not-backed-off ретрай против заведомо неуспешного запроса.
# Новое реальное событие (append/set_status) всё равно поднимет свежий flush
# (см. точки планирования), так что доставка возобновится на след. изменении.
MAX_FLUSH_FAILS = 5
...
class Bubble:
    ...
    model: str = ""
    sent_text: str = ""
    # Счётчик flush'ей подряд, где НИ один адаптер не доставил (см. MAX_FLUSH_FAILS):
    # обнуляется при первой успешной доставке, гасит бесконечный само-респин
    # _flush против устойчиво-недоставляемого бабла. Эфемерный (не персистится).
    fail_streak: int = 0
...
class BubbleManager:
    ...
    async def _flush(self, name: str) -> None:
        ...
        if delivered:
            ...
            bubble.fail_streak = 0
            self._save_live()  # ref появился/обновился — зафиксировать для очистки сирот
        else:
            # Ни один адаптер не доставил (транзиентный сбой ИЛИ устойчиво битый
            # бабл). Копим счётчик — при устойчивом сбое перестаём само-респинить.
            bubble.fail_streak += 1
        ...
        # Но не крутимся вечно на заведомо-недоставляемом: после MAX_FLUSH_FAILS
        # подряд-провалов ждём реального события (оно поднимет свежий flush).
        if (
            self._bubbles.get(name) is bubble
            and bubble.fail_streak < MAX_FLUSH_FAILS
            and self._render_text(bubble, session) != bubble.sent_text
        ):
            bubble.flush_task = asyncio.create_task(self._flush(name))
